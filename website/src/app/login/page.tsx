    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: '#0f172a', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="w-full max-w-md">
        <div className="flex items-center gap-2 justify-center mb-10">
          ...
          <span className="text-[22px] font-bold text-white">Print<span style={{ color: '#8B5CF6' }}>Flow</span></span>
        </div>

        <h1 className="text-center font-extrabold tracking-tight" style={{ fontSize: 'clamp(28px, 4vw, 42px)' }}>
          <span style={{ background: 'linear-gradient(135deg, #8B5CF6, #22C55E)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Zaloguj sie</span>
        </h1>
        <p className="text-center text-slate-400 mt-3 text-base">Wejdz do panelu i zarzadzaj zleceniami.</p>

        <form onSubmit={handleLogin} className="mt-10 rounded-3xl p-8" style={{ background: 'rgba(139,92,246,0.03)', border: '1px solid rgba(139,92,246,0.15)' }}>
          {error && (
          ...
          <p className="mt-5 text-center text-[13px] text-slate-500">
            Nie masz konta? <a href="/register" className="underline" style={{ color: '#8B5CF6' }}>Zaloz farme</a>
          </p>
        </form>
      </div>
    </div>
